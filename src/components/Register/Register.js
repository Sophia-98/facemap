...
class Register extends Component {
    ...
    onPasswordChange = (event) => {
      ...
      onSubmitSignIn = () => {
        ...
        this.setState({ emailError: '', fieldError: '', passwordError: '' }); // Add passwordError
      
        // Check for empty fields
        if (!this.state.email || !this.state.password || !this.state.name) {
          this.setState({ fieldError: 'All fields are required' });
          return;
        }
        ...
        <main className="pa4 black-80">
            <div className="measure">
                ...
                <div className="mt3">
                    ...
                    <input className="pa2 input-reset ba bg-transparent hover-bg-black hover-white w-100" type="text" name="name"  id="user-name" onChange={this.onNameChange}/>
                </div>
                <div className="mt3">
                ...
                <p className="f6 red">{this.state.passwordError}</p>
                )}

                </fieldset>
                {this.state.emailError && (
                    <p className="f6 red">{this.state.emailError}</p>
                    )}
                    {this.state.fieldError && (
                    <p className="f6 red">{this.state.fieldError}</p>
                    )}
